	...
	<table class="spec" width="100%" cellspacing="0" cellpadding="0" border="0">
		...
		<tr>
			<td>
				...
				<div class="cont">
					<ul id="specs">
						<?php $query = new IQuery("spec");$query->where = "is_del = 0";$items = $query->find(); foreach($items as $key => $item){?>
							<li>
								<label><input type="radio" autocomplete="off" name = "spec" value="<?php echo isset($item['id'])?$item['id']:"";?>,<?php echo isset($item['name'])?$item['name']:"";?>,<?php echo isset($item['note'])?$item['note']:"";?>"  onclick="selSpec(this,<?php echo isset($item['id'])?$item['id']:"";?>)" /> <?php echo isset($item['name'])?$item['name']:"";?> [<?php echo isset($item['note'])?$item['note']:"";?>]</label>
							</li>
						<?php }?>
					</ul>
				</div>
...
<script type='text/javascript'>
	...
	function selSpec(curr_spec,id)
	{
		...
		$.ajax({
			   type: "POST",
			   url: "<?php echo IUrl::creatUrl("/block/spec_value_list");?>",
			   data: "id="+id,
			   dataType: "json",
			     ...
			     if(data)
			     {
				     ...
				     for(i=0;i<data['value'].length;i++)
				     {
					     if(data['type']=='1')
					    	...
					    	html += "<li style=\"cursor: default;\"><span><img src='<?php echo IUrl::creatUrl("")."";?>"+data['value'][i]+"' width='45' height='45'/></span></li> ";
				     }
	...
	function insertSpec()
	{
		...
		if(spec)
		{
			...
			html = "<tr>"+
						...
						"</select></td>"+
						"<td><img class='operator' src='<?php echo IUrl::creatUrl("")."views/".$this->theme."/skin/".$this->skin."/images/admin/icon_asc.gif";?>' alt='向上'  onclick='SpecOrder(0,this)' />"+
							"<img class='operator' src='<?php echo IUrl::creatUrl("")."views/".$this->theme."/skin/".$this->skin."/images/admin/icon_desc.gif";?>' alt='向下'  onclick='SpecOrder(-1,this)' />"+
							"<img class='operator' src='<?php echo IUrl::creatUrl("")."views/".$this->theme."/skin/".$this->skin."/images/admin/icon_del.gif";?>' alt='删除'  onclick='delAttr(this)' /></td>"+
					"</tr>";
		}
	...
	function addNewSpec()
	{
		art.dialog.open('<?php echo IUrl::creatUrl("/block/spec_edit");?>', {
			id:'spec_edit',
			width:600,
			height:500,
			lock: true,
	...
	function getSpecList()
	{
		...
		$.ajax({
			   type: "POST",
			   url: "<?php echo IUrl::creatUrl("/block/ajax_spec_list");?>",
			   dataType: "json",
			   success: function(data){
			     if(data)
			     {
