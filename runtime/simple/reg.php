...
<?php $seo_data['title']="用户注册_".$seo_data['title']?>
		...
		<div class="box clearfix">
			...
			<?php $callback = IReq::get('callback')?>
				...
				<table class="form_table f_l">
					<col width="260px" />
					<col />
					<tr><th>邮箱：</th><td><input class="gray" type="text" name='email' value='<?php echo isset($this->email)?$this->email:"";?>' pattern="email" alt="填写正确的邮箱格式" /><label>填写正确的邮箱格式</label></td></tr>
					<tr><th>用户名：</th><td><input class="gray" name='username' value='<?php echo isset($this->username)?$this->username:"";?>' type="text" pattern="^[\w\u0391-\uFFE5]{2,20}$" alt="填写2-20个字符，可以为字数，数字下划线和中文" /><label>请填写用户名，格式为2-20个字符，可以为字数，数字下划线和中文</label></td></tr>
